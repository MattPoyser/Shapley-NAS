...
from tas.initialization import initialize_resnet
    ...
    def __init__(self, block_name, layers, xblocks, xchannels, deep_stem, num_classes, zero_init_residual):
        ...
        if not deep_stem:
            self.layers = nn.ModuleList(
                [ConvBNReLU(xchannels[0], xchannels[1], 7, 2, 3, False, has_avg=False, has_bn=True, has_relu=True)])
            last_channel_idx = 1
        else:
            self.layers = nn.ModuleList(
                [ConvBNReLU(xchannels[0], xchannels[1], 3, 2, 1, False, has_avg=False, has_bn=True, has_relu=True)
                    ,
        ...
        for stage, layer_blocks in enumerate(layers):
            for iL in range(layer_blocks):
                ...
                self.message += "\nstage={:}, ilayer={:02d}/{:02d}, block={:03d}, iCs={:}, oC={:3d}, stride={:}".format(
                    stage, iL, layer_blocks, len(self.layers) - 1, iCs, module.out_dim, stride)
